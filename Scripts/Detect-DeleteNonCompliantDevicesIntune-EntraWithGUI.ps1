...
and optionally delete the corresponding Microsoft Entra device objects.
...
CAUTION:
- Deleting the Entra device object removes BitLocker keys and cannot be undone.
- Hybrid-joined/on-prem synced devices may reappear via sync.
#>
...
if ($deleteAAD) {
    Write-Host "Entra deletion ENABLED — BitLocker keys and device object will be removed." -ForegroundColor Yellow
}
...
if ($deleteAAD) {
  $confirmAAD = Read-Host "Type EXACTLY:  DELETE AAD  to also delete the Entra device objects"
...
foreach ($dev in $selected) {
  ...
  $status = "Unknown"; $err = ""
  $aadDeleteStatus = "Skipped"; $aadDeleteError = ""; $aadObjectId = $null
  ...
  # Optionally delete the Entra device object
  if ($deleteAAD -and $dev.AzureAdDeviceId) {
    try {
      # Map Intune's azureADDeviceId (AAD deviceId) to the directory object Id
      $aad = Get-MgDevice -Filter ("deviceId eq '{0}'" -f $dev.AzureAdDeviceId) -Property "id,deviceId,displayName" -ErrorAction Stop
      if ($aad) {
        $aadObjectId = $aad.Id
  ...
  $results.Add([pscustomobject]@{
    ...
    AAD_DeviceId              = $dev.AzureAdDeviceId   # AAD deviceId (NOT the objectId)
    AAD_ObjectId              = $aadObjectId            # Directory object Id (used for deletion)
...
Write-Host "NOTE: If devices are hybrid-joined or synced from on-prem, they may reappear via sync." -ForegroundColor DarkCyan
